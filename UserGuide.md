...
*   **顶部菜单栏:** 包含文件操作（新建、导入、导出、场景管理）、编辑（撤销/重做占位、删除）、视图（重置、网格）、模拟（模式切换、设置、动画控制）、帮助以及用户登录/状态。
...
*   **场景管理:**
    ...
    *   **文件导入/导出:** 通过顶部“文件”菜单导入/导出 `.json` 场景文件。
...
*   **开始:** 页面加载后画布为空。
*   **添加元件:** 点击左侧工具栏按钮 (如“点光源(激光)”)，鼠标移到画布上会看到预览，单击放置。
*   **观察:** 光线会自动从光源发出并与元件交互。
*   **选中与调整:** 单击画布上的元件，右侧面板自动切换到“属性”标签页。在此修改参数（如光源波长、透镜焦距），观察模拟结果变化。
*   **移动与旋转:** 拖动元件移动，使用角度控制柄或鼠标滚轮（悬停在选中的元件上）旋转。
*   **视图操作:** 滚动滚轮缩放，按住中键平移。通过“视图”菜单重置。
*   **保存场景:** 打开右侧“场景”标签页，点击“另存当前场景...”按钮，输入名称保存到本地浏览器。
*   **加载场景:** 在“场景”标签页点击已保存的场景名称，或通过“文件”菜单导入 `.json` 文件。
*   **删除元件:** 选中元件后按 `Delete` 或 `Backspace` 键，或通过“编辑”菜单操作。

**5. 可用元件一览 (部分)**

*   **光源:** 点光源(激光), 扇形光源, 线光源, 白光光源(模拟光谱, 默认高斯)
*   **透镜:** 薄透镜 (f>0 凸, f<0 凹)
...
*   **快速白光模拟:** (可选性能模式) 切换白光光源的光线生成方式（精确混合 vs. 随机采样）。

**8. 场景管理 (右侧“场景”标签页 / 顶部“文件”菜单)**

*   **本地场景列表:** 显示、加载、删除保存在当前浏览器中的场景。
*   **另存当前场景:** 将当前画布布局命名并保存到本地浏览器存储。
...
*   **保存场景:** 经常使用“场景”标签页的“另存为”功能保存你的工作。
*   **查看信息:** “信息”标签页和“帮助”菜单提供版本和基本指南。

**10. 未来展望**

交互式光学实验室将持续开发，未来可能加入：更精确的物理模型（高斯光束传输、衍射细节、像差）、更多元件、撤销/重做、单位系统、云同步、协作、更丰富的可视化选项等。

---

**说明：**

*   **版本号更新为 v1.1:** 反映了界面的重大重构和新功能的加入。
*   **界面布局描述:** 增加了对新界面（顶部菜单、标签页）的描述。
*   **操作流程更新:** 更新了快速入门和特性描述，以匹配新的菜单项和标签页交互。
*   **设置和场景管理:** 将这两部分的功能描述移到了对应的标签页下。
*   **占位符:** 保留了对未来功能（如撤销/重做、云功能）的提及。
*   **措辞调整:** 使语言更流畅，更符合用户指南的风格。
