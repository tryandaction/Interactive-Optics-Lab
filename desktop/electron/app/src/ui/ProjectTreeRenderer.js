...
export class ProjectTreeRenderer {
    ...
    async render() {
        ...
        const tree = await this.projectManager.getProjectTree();
    ...
    renderNode(node, isRoot = false) {
        ...
        if (node.type === 'scene') {
            ...
            const isActive = currentScene && node.id === currentScene.id;
            const isModified = currentScene?.isModified && isActive;
            ...
            return `
                <div class="tree-node scene-node ${isActive ? 'active' : ''}" 
                    ...
                    <span class="node-name">${isModified ? '• ' : ''}${this.escapeHtml(node.name)}</span>
                </div>
            `;
        }

        const isProject = node.type === 'project';
        const nodeId = isProject ? node.id : node.id;
        const isExpanded = this.expandedNodes.has(nodeId);
        const icon = isProject
            ? this.getStorageIcon(this.projectManager.getCurrentProject()?.storageMode)
            : (isExpanded ? '📂' : '📁');
        const toggle = isExpanded ? '▼' : '▶';
        const syncIndicator = isProject && this.projectManager.getCurrentProject()?.storageMode === 'github'
            ? '<span class="sync-indicator" title="GitHub 项目">🔗</span>'
            : '';

        const children = node.children || [];
        const emptyLabel = isProject ? '暂无场景' : '空文件夹';
            ...
            ? (children.length
                ...
                : `
                    <div class="tree-node empty-node">
                        <span class="node-icon">📄</span>
                        <span class="node-name placeholder">${emptyLabel}</span>
                    </div>
                `)
            : '';

        return `
            <div class="tree-node ${isProject ? 'project-node' : 'directory-node'} ${isExpanded ? 'expanded' : ''}" 
                 data-id="${nodeId}" 
                 data-type="${isProject ? 'project' : 'directory'}">
    ...
    bindNodeEvents() {
        ...
        this.container.querySelectorAll('.project-node, .directory-node').forEach(node => {
            node.addEventListener('dragenter', (e) => this.handleDragEnter(e, node));
            node.addEventListener('dragleave', (e) => this.handleDragLeave(e, node));
            node.addEventListener('dragover', (e) => this.handleDragOver(e, node));
            node.addEventListener('drop', (e) => this.handleDrop(e, node));
        });
    }
    ...
    async handleNodeClick(e, node) {
        ...
        if (type === 'project' || type === 'directory') {
            // 切换展开/折叠
    ...
    getContextMenuItems(type, id) {
        ...
        if (type === 'directory') {
            return [
                { label: '新建场景', action: 'newSceneInDir' },
                { label: '新建子文件夹', action: 'newSubFolder' },
                { separator: true },
                { label: '重命名', action: 'renameDir' },
                { label: '删除', action: 'deleteDir' }
            ];
        }
        ...
        try {
            switch (action) {
                case 'newScene':
                    ...
                    break;
                case 'newSceneInDir':
                    this.emit('newSceneInDirectoryRequested', { path: id });
                    break;
                case 'newSubFolder':
                    this.emit('newSubFolderRequested', { path: type === 'directory' ? id : '' });
                    break;
                case 'sync':
                ...
                case 'rename':
                    ...
                    break;
                case 'renameDir':
                    this.emit('renameDirectoryRequested', { path: id });
                    break;
                case 'close':
                ...
                case 'delete':
                    ...
                    break;
                case 'deleteDir':
                    this.emit('deleteDirectoryRequested', { path: id });
                    break;
            }
        } catch (err) {
    ...
    handleDragEnter(e, node) {
        if (!this._isValidDropTarget(node)) {
            return;
        }
        e.preventDefault();
        this._setDropTarget(node);
    }
    ...
    handleDrop(e, node) {
        ...
        const targetPath = type === 'project' ? '' : node.dataset.id;

        if (draggedId) {
            this.emit('sceneMoved', { sceneId: draggedId, targetPath });
        }
    ...
    _isValidDropTarget(node) {
        const type = node?.dataset?.type;
        return type === 'project' || type === 'directory';
    }
    ...
    _clearDropTargets() {
        this.container.querySelectorAll('.tree-node.drop-target').forEach(el => {
