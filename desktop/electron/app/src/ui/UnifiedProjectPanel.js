...
export class UnifiedProjectPanel {
    ...
    bindEvents() {
        ...
        if (treeContainer) {
            ...
            treeContainer.addEventListener('projectTree:duplicateSceneRequested', (e) => {
                this.showDuplicateSceneModal(e.detail.id);
            });

            treeContainer.addEventListener('projectTree:newSubFolderRequested', (e) => {
                this.showCreateSubFolderModal({ parentPath: e.detail?.path || '' });
            });

            treeContainer.addEventListener('projectTree:newSceneInDirectoryRequested', (e) => {
                this.showCreateSceneModal({ directoryPath: e.detail?.path || '' });
            });

            treeContainer.addEventListener('projectTree:renameDirectoryRequested', (e) => {
                this.showRenameDirectoryModal(e.detail?.path || '');
            });

            treeContainer.addEventListener('projectTree:deleteDirectoryRequested', (e) => {
                this.showDeleteDirectoryConfirm(e.detail?.path || '');
            });

            treeContainer.addEventListener('projectTree:sceneMoved', async (e) => {
                const sceneId = e.detail?.sceneId;
                const targetPath = e.detail?.targetPath || '';
                if (!sceneId) return;

                try {
                    await this.projectManager.moveScene(sceneId, targetPath);
                    this.showNotification('场景已移动', 'success');
                    if (this.treeRenderer) {
                        this.treeRenderer.render();
                    }
                } catch (err) {
                    this.showNotification(`移动失败: ${err.message}`, 'error');
                }
            });
        }
    ...
    showCreateSceneModal(options = {}) {
        ...
        const locationLabel = directoryPath
            ? `路径: <strong>${this.escapeHtml(directoryPath)}</strong>`
        ...
        modal.querySelector('#create-scene-form')?.addEventListener('submit', async (e) => {
            ...
            try {
                // 创建场景
                const scene = await this.projectManager.createScene(name, { directoryPath });
    ...
    showDuplicateSceneModal(sceneId) {
        const project = this.projectManager.getCurrentProject();
        ...
        const suggestedName = `${scene.name} 副本`;
        const modal = this.createModal('duplicate-scene-modal', `
            <h3>复制场景</h3>
            <form id="duplicate-scene-form">
                <div class="form-group">
                    <label for="duplicate-scene-name">新场景名称</label>
                    <input type="text" id="duplicate-scene-name" required value="${this.escapeHtml(suggestedName)}">
                </div>
                <div class="form-actions">
                    <button type="button" class="btn-cancel">取消</button>
                    <button type="submit" class="btn-primary">确定</button>
        ...
        modal.querySelector('#duplicate-scene-form')?.addEventListener('submit', async (e) => {
            e.preventDefault();
            const newName = modal.querySelector('#duplicate-scene-name').value.trim();
            if (!newName) return;

            try {
                await this.projectManager.duplicateScene(sceneId, { name: newName, open: false });
                this.closeModal(modal);
                this.showNotification('场景已复制', 'success');
            } catch (err) {
    ...
    showCreateSubFolderModal(options = {}) {
        const project = this.projectManager.getCurrentProject();
        if (!project) {
            this.showNotification('请先打开项目', 'warning');
            return;
        }

        const parentPath = options.parentPath || '';
        const locationLabel = parentPath
            ? `父目录: <strong>${this.escapeHtml(parentPath)}</strong>`
            : `项目: <strong>${this.escapeHtml(project.name)}</strong>`;

        const modal = this.createModal('create-subfolder-modal', `
            <h3>新建子文件夹</h3>
            <p style="color: var(--text-color-light); font-size: 13px; margin-bottom: 15px;">
                ${locationLabel}
            </p>
            <form id="create-subfolder-form">
                <div class="form-group">
                    <label for="subfolder-name">文件夹名称</label>
                    <input type="text" id="subfolder-name" required placeholder="例如：实验记录">
                </div>
                <div class="form-actions">
                    <button type="button" class="btn-cancel">取消</button>
                    <button type="submit" class="btn-primary">确定</button>
        ...
        modal.querySelector('#create-subfolder-form')?.addEventListener('submit', async (e) => {
            e.preventDefault();
            const name = modal.querySelector('#subfolder-name').value.trim();
            if (!name) return;

            try {
                await this.projectManager.createSubProject(name, parentPath);
                this.closeModal(modal);
                this.showNotification('子文件夹已创建', 'success');
                if (this.treeRenderer) {
                    this.treeRenderer.render();
                }
            } catch (err) {
                this.showNotification(`创建失败: ${err.message}`, 'error');
            }
        });
    ...
    showRenameDirectoryModal(dirPath) {
        if (!dirPath) {
            this.showNotification('目录路径无效', 'warning');
            return;
        }
        ...
        const modal = this.createModal('rename-directory-modal', `
            <h3>重命名文件夹</h3>
            <p style="color: var(--text-color-light); font-size: 13px; margin-bottom: 15px;">
                当前路径: <strong>${this.escapeHtml(dirPath)}</strong>
            </p>
            <form id="rename-directory-form">
                <div class="form-group">
                    <label for="directory-name">新名称</label>
                    <input type="text" id="directory-name" required value="${this.escapeHtml(currentName)}">
                </div>
                <div class="form-actions">
                    <button type="button" class="btn-cancel">取消</button>
                    <button type="submit" class="btn-primary">确定</button>
        ...
        modal.querySelector('#rename-directory-form')?.addEventListener('submit', async (e) => {
            e.preventDefault();
            const newName = modal.querySelector('#directory-name').value.trim();
            if (!newName) return;

            try {
                await this.projectManager.renameDirectory(dirPath, newName);
                this.closeModal(modal);
                this.showNotification('文件夹已重命名', 'success');
                if (this.treeRenderer) {
                    this.treeRenderer.render();
                }
            } catch (err) {
    ...
    async showDeleteDirectoryConfirm(dirPath) {
        if (!dirPath) {
            this.showNotification('目录路径无效', 'warning');
            return;
        }

        const confirmed = confirm(`确定要删除文件夹吗？\n该操作将递归删除：${dirPath}`);
        if (!confirmed) return;

        try {
            await this.projectManager.deleteDirectory(dirPath);
