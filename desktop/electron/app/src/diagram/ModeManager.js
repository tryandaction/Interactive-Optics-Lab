...
export function getModeManager(options = {}) {
    ...
    // ModeManager 是单例，但不同模块初始化顺序不稳定：
    // 例如 UI 先创建 ModeSwitcher（无 stateProvider / onModeChange），
    // 之后 DiagramModeIntegration 才提供 stateProvider 与回调。
    // 因此这里需要把后续传入的配置“合并”到已存在实例中，避免绘图模式逻辑不生效。
    if (options && typeof options === 'object') {
        if (options.stateProvider && !modeManagerInstance.stateProvider) {
            modeManagerInstance.stateProvider = options.stateProvider;
        }
        if (typeof options.onModeChange === 'function') {
            modeManagerInstance.onModeChange(options.onModeChange);
        }
    }
