...
function serializeSurfaceNormal(trace) {
    const normal = trace.surfaceNormal;
    if (!Number.isFinite(normal?.x) || !Number.isFinite(normal?.y)) return null;
    const magnitude = Math.hypot(normal.x, normal.y);
    return magnitude > 0 ? { x: normal.x / magnitude, y: normal.y / magnitude } : null;
}
    ...
    static fromTraceRecords(components = [], traces = []) {
        ...
        traces.forEach((trace, index) => {
            ...
            graph.addEdge({
                ...
                interactionType: trace.interactionType || null,
                surfaceId: trace.surfaceId || null,
                surfaceNormal: serializeSurfaceNormal(trace),
                wavelengthNm,
                frequencyOffsetHz,
