...
class Polarizer extends OpticalComponent {
    ...
    // --- REPLACEMENT for Polarizer.interact with Logging ---
    interact(ray, intersectionInfo, RayClass) {
        const hitPoint = intersectionInfo.point;
        const transmissionAxis = this.transmissionAxisRad; // Transmission axis in radians
        const incidentPolarization = ray.polarizationAngle; // Can be number, null, or 'circular'

        console.log(`[Polarizer ${this.id}] Interact: Input Pol =`, incidentPolarization, `(Type: ${typeof incidentPolarization}), Axis = ${(transmissionAxis * 180 / Math.PI).toFixed(1)} deg`); // DEBUG

        let transmittedIntensity = 0;
        const transmittedPolarizationAngle = transmissionAxis; // Output is always linearly polarized along the axis

        if (typeof incidentPolarization === 'number') {
            // Incident is linearly polarized: Malus's Law
            const angleDiff = incidentPolarization - transmissionAxis;
            const cosTheta = Math.cos(angleDiff);
            transmittedIntensity = ray.intensity * (cosTheta * cosTheta);
            console.log(`  -> Input Linear: AngleDiff=${(angleDiff * 180 / Math.PI).toFixed(1)} deg, cos^2=${(cosTheta * cosTheta).toFixed(3)}, Output Intensity=${transmittedIntensity.toExponential(3)}`); // DEBUG
        } else if (incidentPolarization === null) {
            // Incident is unpolarized: Intensity halves, becomes linearly polarized
            transmittedIntensity = ray.intensity / 2.0;
            console.log(`  -> Input Unpolarized: Output Intensity=${transmittedIntensity.toExponential(3)}`); // DEBUG
        } else if (incidentPolarization === 'circular') {
            // Incident is circularly polarized: Intensity halves, becomes linearly polarized
            transmittedIntensity = ray.intensity / 2.0;
            console.log(`  -> Input Circular: Output Intensity=${transmittedIntensity.toExponential(3)}`); // DEBUG
        } else {
            console.warn(`  -> Unknown polarization type: ${incidentPolarization}. Blocking light.`); // DEBUG
            transmittedIntensity = 0; // Block unknown states
        }


        // Apply ignoreDecay AFTER physical calculation (current behavior)
        // ... (logic for ignoreDecay if needed, but usually polarizers reduce intensity physically)
        ...
        if (transmittedIntensity >= ray.minIntensityThreshold) {
            ...
            try {
                transmittedRay = new RayClass(
                    ...
                    ray.sourceId,
                    transmittedPolarizationAngle, // Set output polarization angle
                    ray.ignoreDecay,
                    ray.history.concat([newOrigin.clone()])
                );
                console.log(`  -> Created Transmitted Ray: Pol = ${(transmittedRay.polarizationAngle * 180 / Math.PI).toFixed(1)} deg`); // DEBUG
            } catch (e) { console.error(`Error creating transmitted Ray in Polarizer (${this.id}):`, e); }
        } else {
            console.log(`  -> Transmitted intensity below threshold.`); // DEBUG
        }
...
class BeamSplitter extends OpticalComponent {
    ...
    draw(ctx) { // Same as before - draws 'P' indicator
        if (!(this.p1 instanceof Vector) || !(this.p2 instanceof Vector)) return;
        ...
        if (this.type === 'PBS') {
            ...
            const offsetVec = this.normal.multiply(8);
            ctx.fillText('P', this.pos.x + offsetVec.x, this.pos.y + offsetVec.y);
        }
    }
    ...
    intersect(rayOrigin, rayDirection) { // Same as before
        if (!(this.p1 instanceof Vector) || !(this.p2 instanceof Vector)) return [];
        const v1 = rayOrigin.subtract(this.p1), v2 = this.p2.subtract(this.p1);
        const v3 = new Vector(-rayDirection.y, rayDirection.x);
        const d_v2_v3 = v2.dot(v3); if (Math.abs(d_v2_v3) < 1e-9) return [];
    ...
    _interactPBS(ray, intersectionInfo, RayClass) {
        ...
        const pAxis = this._getP_AxisDirection();
        const sAxis = this._getS_AxisDirection();
        const pAxisAngle = pAxis.angle();
        const sAxisAngle = sAxis.angle();

        let transmittedIntensity = 0; // Will become p-polarized
        let reflectedIntensity = 0;   // Will become s-polarized
        const transmittedPolarizationAngle = pAxisAngle;
        const reflectedPolarizationAngle = sAxisAngle;

        // Ideal Extinction: Assume perfect transmission for p, perfect reflection for s for now.
        // We use pbsUnpolarizedReflectivity to adjust the *balance* if the input is mixed/unpolarized.
        const ideal_pTransmissionCoeff = 1.0; // Perfect T for p
        const ideal_sReflectionCoeff = 1.0; // Perfect R for s

        if (ray.isPolarized()) {
            const incidentAngle = ray.getPolarizationAngle();
            let angleRelToP = incidentAngle - pAxisAngle;
            angleRelToP = Math.atan2(Math.sin(angleRelToP), Math.cos(angleRelToP));

            const cosThetaP = Math.cos(angleRelToP);
            const sinThetaP = Math.sin(angleRelToP); // Component along s-axis

            const pComponentIntensity = incidentIntensity * (cosThetaP * cosThetaP);
            const sComponentIntensity = incidentIntensity * (sinThetaP * sinThetaP);

            // Apply ideal extinction first
            let idealTransmitted = pComponentIntensity * ideal_pTransmissionCoeff; // Ideally only p transmits
            let idealReflected = sComponentIntensity * ideal_sReflectionCoeff;   // Ideally only s reflects

            // --- Adjust Balance ---
            // If the ideal split isn't 50/50 (pComponent != sComponent), how does pbsUnpolarizedReflectivity influence it?
            // This is tricky. A simple model: scale the outputs slightly towards the pbsUnpolarizedReflectivity target,
            // while trying to conserve energy. Let targetR = this.pbsUnpolarizedReflectivity.
            // Let currentR = idealReflected / (idealReflected + idealTransmitted) || 0.5;
            // We could nudge the results, but let's keep it simpler for now:
            // Let the pbsUnpolarizedReflectivity *primarily* define the split for UNPOLARIZED light,
            // and assume near-ideal extinction for purely s or p polarized light.
            // For mixed states, we *could* use it to slightly bias the cos^2/sin^2 split, but let's omit that complexity for now.

            // Let's stick to the simpler cos^2 / sin^2 split for polarized input for now.
            transmittedIntensity = idealTransmitted;
            reflectedIntensity = idealReflected;


        } else {
            // Unpolarized light: Split according to pbsUnpolarizedReflectivity
            reflectedIntensity = incidentIntensity * this.pbsUnpolarizedReflectivity;
            transmittedIntensity = incidentIntensity * (1.0 - this.pbsUnpolarizedReflectivity);
        }

        // Create Reflected Ray (s-polarized)
        if (reflectedIntensity >= ray.minIntensityThreshold) {
            const N = surfaceNormal; const I = incidentDirection;
            const dot_I_N = I.dot(N);
            const reflectedDirection = I.subtract(N.multiply(2 * dot_I_N)).normalize();
            ...
            try {
                ...
                const reflectedRay = new RayClass(
                    ...
                    ray.phase + Math.PI, nextBounces, ray.mediumRefractiveIndex,
                    ray.sourceId,
                    reflectedPolarizationAngle, // Force s-polarization
                    ray.ignoreDecay,
                    ray.history.concat([reflectOrigin.clone()]) // Pass history + new origin
                );
                newRays.push(reflectedRay);
            } catch (e) { console.error(`PBS (${this.id}): Error creating reflected (s) ray:`, e); }
        }

        // Create Transmitted Ray (p-polarized)
        if (transmittedIntensity >= ray.minIntensityThreshold) {
            const transmittedDirection = incidentDirection;
            const transmitOrigin = hitPoint.add(transmittedDirection.multiply(1e-6));
            try {
                ...
                const transmittedRay = new RayClass(
                    ...
                    ray.phase, nextBounces, ray.mediumRefractiveIndex,
                    ray.sourceId,
                    transmittedPolarizationAngle, // Force p-polarization
                    ray.ignoreDecay,
                    ray.history.concat([transmitOrigin.clone()]) // Pass history + new origin
                );
                newRays.push(transmittedRay);
            } catch (e) { console.error(`PBS (${this.id}): Error creating transmitted (p) ray:`, e); }
        }
...
class HalfWavePlate extends WavePlate {
    ...
    // --- REPLACEMENT for HalfWavePlate.interact (Higher Precision) ---
    interact(ray, intersectionInfo, RayClass) {
        const hitPoint = intersectionInfo.point;
        const newRays = [];
        const nextBounces = ray.bouncesSoFar + 1;
        const incidentIntensity = ray.intensity;
        let outputPolarization = ray.polarizationAngle; // Default pass-through

        // console.log(`[HWP ${this.id}] Interact: Input Pol=`, ray.polarizationAngle, ` FastAxis=${(this.fastAxisRad * 180 / Math.PI).toFixed(1)}`); // Keep logging if needed

        if (typeof ray.polarizationAngle === 'number' && !isNaN(ray.polarizationAngle)) {
            const incidentAngleRad = ray.polarizationAngle; // Already normalized in Ray class usually
            const fastAxis = this.fastAxisRad; // Assume this is also kept normalized

            // Calculate output angle: 2 * fastAxis - incidentAngleRad
            const outputAngleRad_raw = 2 * fastAxis - incidentAngleRad;
            // *** Use atan2 for robust normalization to [-PI, PI] ***
            outputPolarization = Math.atan2(Math.sin(outputAngleRad_raw), Math.cos(outputAngleRad_raw));

            // console.log(`  -> Input Linear: Calc Output = ${(outputPolarization * 180 / Math.PI).toFixed(2)} deg`); // Log with more precision
        } else {
            // console.log(`  -> Input not linear (${ray.polarizationAngle}). Passing through.`);
        }

        if (incidentIntensity >= ray.minIntensityThreshold) {
            const newDirection = ray.direction;
            const newOrigin = hitPoint.add(newDirection.multiply(1e-6));
            try {
                const transmittedRay = new RayClass(
                    newOrigin, newDirection, ray.wavelengthNm, incidentIntensity,
                    ray.phase, nextBounces, ray.mediumRefractiveIndex, ray.sourceId,
                    outputPolarization, ray.ignoreDecay, ray.history.concat([newOrigin.clone()])
                );
                // console.log(`  -> Created Ray: Pol=`, transmittedRay.polarizationAngle); // Log state
                if (!transmittedRay.terminated) newRays.push(transmittedRay);
            } catch (e) { console.error(`HWP (${this.id}) Error creating transmitted ray:`, e); }
        }
...
class QuarterWavePlate extends WavePlate {
    ...
    // --- REPLACEMENT for QuarterWavePlate.interact (Higher Precision) ---
    interact(ray, intersectionInfo, RayClass) {
        const hitPoint = intersectionInfo.point;
        const newRays = [];
        const nextBounces = ray.bouncesSoFar + 1;
        ...
        // *** Use smaller tolerance (e.g., 0.1 degrees in radians) ***
        const angleTolerance = 0.1 * Math.PI / 180; // Approx 0.0017 radians

        // console.log(`[QWP ${this.id}] Interact: Input Pol=`, ray.polarizationAngle, ` FastAxis=${(fastAxis * 180 / Math.PI).toFixed(1)}`); // Keep logging

        if (typeof ray.polarizationAngle === 'number' && !isNaN(ray.polarizationAngle)) {
            const incidentAngleRad = ray.polarizationAngle;
            // Calculate angle difference robustly using atan2
            const angleDiffRaw = incidentAngleRad - fastAxis;
            const angleDiffNormalized = Math.atan2(Math.sin(angleDiffRaw), Math.cos(angleDiffRaw)); // Difference in [-PI, PI]
            const angleDiffPositive = Math.abs(angleDiffNormalized); // Absolute difference [0, PI]

            // console.log(`  -> Input Linear: Normalized Angle Diff = ${(angleDiffPositive * 180/Math.PI).toFixed(2)} deg`); // Log with more precision

            // Check if input is approx +/- 45 degrees using smaller tolerance
            if (Math.abs(angleDiffPositive - DEG45) < angleTolerance) {
                outputPolarization = 'circular';
                // console.log(`    --> Angle near 45 deg. Outputting 'circular'.`); // Log if needed
            }
            // Check if input is parallel or perpendicular using smaller tolerance
            else if (angleDiffPositive < angleTolerance || Math.abs(angleDiffPositive - Math.PI) < angleTolerance) {
                outputPolarization = incidentAngleRad; // Pass through linear
                // console.log(`    --> Angle near 0/180 deg. Passing linear through.`); // Log if needed
            } else if (Math.abs(angleDiffPositive - Math.PI / 2) < angleTolerance) {
                outputPolarization = incidentAngleRad; // Pass through linear
                // console.log(`    --> Angle near 90 deg. Passing linear through.`); // Log if needed
            }
            else {
                outputPolarization = incidentAngleRad; // Simplification: elliptical -> linear
                // console.log(`    --> Angle is general. Outputting *elliptical* (SIMPLIFIED as input linear angle).`); // Log if needed
            }
        } else if (ray.polarizationAngle === 'circular') {
            // Circular in -> Linear out at 45 deg to fast axis
            outputPolarization = fastAxis + DEG45;
            outputPolarization = Math.atan2(Math.sin(outputPolarization), Math.cos(outputPolarization)); // Normalize
            // console.log(`  -> Input Circular: Outputting Linear at ${(outputPolarization * 180/Math.PI).toFixed(2)} deg.`); // Log with more precision
        } else {
            // console.log(`  -> Input Unpolarized or Elliptical. Passing through.`);
        }
        ...
        if (incidentIntensity >= ray.minIntensityThreshold) {
            ...
            try {
                ...
                // console.log(`  -> Created Ray: Pol State=`, transmittedRay.polarizationAngle); // Log state
                if (!transmittedRay.terminated) newRays.push(transmittedRay);
            } catch (e) { console.error(`QWP (${this.id}) Error creating transmitted ray:`, e); }
        }
