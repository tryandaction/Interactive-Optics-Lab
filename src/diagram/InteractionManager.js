    ...
    copySelection(links = []) {
        ...
        if (items.length > 0) {
            if (links.length === 0 && typeof window !== 'undefined') {
                ...
                const rayLinkManager = diagramIntegration?.isDiagramMode?.()
                    ? diagramIntegration.getModule?.('rayLinkManager')
                    : null;
                if (rayLinkManager?.getAllLinks) {
                    links = rayLinkManager.getAllLinks();
                }
            }
