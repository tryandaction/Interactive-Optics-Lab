...
export function registerExtendedIcons() {
  ...
  console.log(`ExtendedIconLibrary: Registered ${Object.keys(EXTENDED_SOURCES).length +
...
/**
 * Extended Icon Library Part 1: Sources and Mirrors
 */
export function registerExtendedIconsPart1(iconManager) {
  // Register Sources
  Object.entries(EXTENDED_SOURCES).forEach(([type, definition]) => {
    iconManager.registerIcon(type, definition);
  });
  ...
  console.log(`ExtendedIconLibrary Part1: Registered ${Object.keys(EXTENDED_SOURCES).length + Object.keys(EXTENDED_MIRRORS).length
    } icons (Sources + Mirrors)`);
